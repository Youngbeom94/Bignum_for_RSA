#include "Bignumber.h"
#if 1
int main() //! Addition이 잘 작동되는지 확인하는 함수 (Verfication)
{
...
#endif
#if 0
//!performance test Addition --> Addition 성능을 체크해주는 함수 이다.
int main()
{
    clock_t start, end;
    ...
    start = clock(); //*두번째 측정시작
     while ((i >= 0)) //! 두번째 while문에서는 A,B 변수를 generate하고 Addition을 통해 C를 구하는 시간, 그리고 해제하는 시간을 측정한다.
    {
        i--;
        bigint_st_ptr A = NULL;
        bigint_st_ptr B = NULL;
    ...
    printf("\nElapsed time with Addition: %lf\n", cpu_time_used2 - cpu_time_used1);
    //! 두번째 구한 시간에서 첫번째 구한 시간을 뺴주게 되면, 근사적으로 Addition을 수행하는 시간을 구해줄 수 있다.
    return 0;
}
#endif
